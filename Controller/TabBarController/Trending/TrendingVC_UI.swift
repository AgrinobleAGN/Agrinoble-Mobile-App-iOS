...
class TrendingVC_UI: UIViewController, UITabBarControllerDelegate, createLiveDelegate, loadEventDelegate {
    ...
    var weatherLocation:[String:Any]?
    ...
    func fetchWeather(){
        ...
        case .online(.wwan),.online(.wiFi):
            ...
            performUIUpdatesOnMain {
                WeatherManager.sharedInstance.getWeather() { (Success, Error) in
                                    if Success != nil {
//                        self.blogs = success?.blogs
                        let suc = Success
                        guard let location = suc!["location"] as? [String:Any] else {return}
                        self.weatherLocation = location
                        guard let current = suc!["current"] as? [String:Any] else {return}
                        guard let condition = current["condition"] as? [String:Any] else {return}
                        self.weatherCondition = condition
                        self.wind = current
//                        guard let text = condition["text"] as? String else {return}
//                        guard let wind = current["wind_mph"] as? String else {return}
//                        guard let humidity = current["humidity"] as? String else {return}
                                   
                        guard let forecast = suc!["forecast"] as? [String:Any] else {return}
                        guard let forecastday = forecast["forecastday"] as? [[String:Any]] else {return}
//                        guard let hour = forecastday["hour"] as? [String:Any] else {return}
//                        guard let rain = hour["chance_of_rain"] as? String else {return}
...
//            }
        case 2 :
            ...
            cell.setupUI()
            cell.locationLabel.text = "\(weatherLocation?["name"] ?? ""), \(weatherLocation?["country"] ?? "") \(weatherLocation?["localtime"] ?? "")"
            cell.weatherConditionLabel.text = "\(weatherCondition?["text"] ?? "")"
            cell.windLabel.text = "\(wind?["wind_mph"] ?? "")"
            cell.humidityLAbel.text = "\(wind?["humidity"] ?? "")"
            return cell
